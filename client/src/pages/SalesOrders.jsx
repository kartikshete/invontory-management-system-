...
export default function SalesOrders() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/sales-orders', { params: { search, page, limit: 10 } });
    ...
    setFormData((prev) => {
      ...
      if (field === 'product') {
        ...
        if (found) {
          ...
          items[index].unitPrice = found.sellingPrice || 0;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.customer) return toast.error('Customer is required');
    ...
    try {
      if (editingOrder) {
        await api.put(`/sales-orders/${editingOrder._id}`, payload);
                    ...
                    <td className="table-cell">
                      <div>
                        ...
                        {order.customer?.company && (
                          <p className="text-xs text-slate-400">{order.customer.company}</p>
                        )}
                      </div>
                    </td>
